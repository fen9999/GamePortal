...
public class EasyFontTextMesh : MonoBehaviour {
	...
	void RefreshMesh(bool _updateTexureInfo)
	{
		...
		{
			//We draw the text 4 times in the back of the main text	with an offset
			
			//Right
			ResetHelperVariables();
			foreach (char iteratorChar in textChars)
			{
				CreateCharacter(iteratorChar, characterPosition, Vector3.right*_privateProperties.outLineWidth, _privateProperties.outlineColor, _privateProperties.outlineColor);	
				characterPosition++;
			}
			SetAlignment(alignmentPass++);
			//left
			...
			foreach (char iteratorChar in textChars)
			{
				CreateCharacter(iteratorChar, characterPosition, Vector3.left*_privateProperties.outLineWidth, _privateProperties.outlineColor, _privateProperties.outlineColor);	
				characterPosition++;
			}
			SetAlignment(alignmentPass++);
			//Up
			...
			foreach (char iteratorChar in textChars)
			{
				CreateCharacter(iteratorChar, characterPosition, Vector3.up*_privateProperties.outLineWidth, _privateProperties.outlineColor, _privateProperties.outlineColor);	
				characterPosition++;
			}
			SetAlignment(alignmentPass++);
			//Down
			...
			foreach (char iteratorChar in textChars)
			{
				CreateCharacter(iteratorChar, characterPosition, Vector3.down*_privateProperties.outLineWidth, _privateProperties.outlineColor, _privateProperties.outlineColor);	
				characterPosition++;
			}
			SetAlignment(alignmentPass++);
		}
	...
	void SetAlignment(int _pass)
	{
		...
		int vertexPassOffset = _pass*textChars.Length*4; // We have to align the outline,shadow and main. The
